#!/usr/bin/env python3
"""
Process MultiNLI conformal result zip files and generate the
calibration-size detectability analysis.
...
The computation flow is intentionally kept the same as in the original
`data_plot.py`: the script reads the arrays directly from each zip, performs
stratified calibration subsampling, evaluates pooled-threshold group-wise
miscoverage on the fixed test split, and writes plots + CSV summaries to
./data_plot_multinli_output.

Main outputs:
  - data_plot_multinli_output/full_metrics_summary.csv
  - data_plot_multinli_output/subsample_summary.csv
  - data_plot_multinli_output/detectability_scaling.csv
  - data_plot_multinli_output/figure_floor_vs_n.pdf
  - data_plot_multinli_output/figure_snr_vs_n.pdf
  - data_plot_multinli_output/figure_floor_inflation_vs_n.pdf
  - data_plot_multinli_output/figure_sigma_inflation_vs_n.pdf
  - data_plot_multinli_output/figure_normalized_collapse_snr.pdf
"""
...
def plot_floor_vs_n(df: pd.DataFrame, out_path: Path) -> None:
    ...
    for score, sub in ordered_group_items(df):
        ...
        line, = plt.plot(x, y, marker="o", linewidth=2)
        color = line.get_color()
        plt.fill_between(x, np.maximum(y - s, 0.0), y + s, color=color, alpha=0.18)
        floor_line = plt.axhline(tf, linestyle="--", linewidth=1.3, color=color)
    ...
    plt.ylabel("RMS group miscoverage on fixed test split")
    plt.title("Empirical floor vs calibration size")
    plt.grid(True, alpha=0.3)
    plt.legend(handles, labels, frameon=False, fontsize=11, ncol=2)
    plt.tight_layout()
...
def plot_snr_vs_n(df: pd.DataFrame, out_path: Path) -> None:
    ...
    for score, sub in ordered_group_items(df):
        sub = sub.sort_values("n_cal")
        # Exclude the full-cal deterministic endpoint (sd=0, hence SNR=inf).
        sub = sub[np.isfinite(sub["snr_true_floor_over_sd"])].copy()
        if len(sub) == 0:
            continue
        xvals = sub["n_cal"].to_numpy(dtype=float)
        tick_values.extend(xvals.tolist())
        line, = plt.plot(xvals, sub["snr_true_floor_over_sd"], marker="o", linewidth=4.4, markersize=8.5)
        handles.append(line)
        labels.append(score)
    thresh = plt.axhline(1.0, linestyle="--", linewidth=1.4, color="black")
    handles.append(thresh)
    labels.append("SNR = 1 threshold")
    plt.xscale("log")
    if tick_values:
        tick_values = sorted(set(int(x) for x in tick_values))
        plt.xticks(tick_values, [str(x) for x in tick_values])
        plt.minorticks_off()
    plt.xlabel("Calibration size")
    plt.ylabel(r"SNR = true floor / sd(empirical floor)")
    plt.title("Detectability of the structural floor",fontsize=25)
    plt.grid(True, alpha=0.3)
    plt.legend(handles, labels, frameon=False, fontsize=19)
    plt.tight_layout()
    plt.savefig(out_path, dpi=220)
    plt.close()
...
def plot_ratio_vs_n(df: pd.DataFrame, ycol: str, ylabel: str, title: str, out_path: Path) -> None:
    ...
    for score, sub in ordered_group_items(df):
        sub = sub.sort_values("n_cal")
        line, = plt.plot(sub["n_cal"], sub[ycol], marker="o", linewidth=2)
        handles.append(line)
        labels.append(score)
    ref = plt.axhline(1.0, linestyle="--", linewidth=1.2, color="black")
    handles.append(ref)
    ...
    plt.title(title)
    plt.grid(True, alpha=0.3)
    plt.legend(handles, labels, frameon=False, fontsize=11)
    plt.tight_layout()
    plt.savefig(out_path, dpi=220)
    plt.close()
...
def plot_normalized_collapse(sub_df: pd.DataFrame, full_df: pd.DataFrame, eta: float, out_path: Path) -> None:
    ...
    for score, sub in ordered_group_items(merged):
        ...
        if len(sub) == 0:
            continue
        line, = plt.plot(sub["normalized_n"], sub["snr_true_floor_over_sd"], marker="o", linewidth=2)
        handles.append(line)
        labels.append(score)
    thresh = plt.axhline(1.0, linestyle="--", linewidth=1.2, color="black")
    handles.append(thresh)
    labels.append("SNR = 1 threshold")
    plt.xscale("log")
    plt.xlabel(r"Normalized calibration size: $n m_{eff}(q)^2 \sigma_\Delta^2 / (K\log(K/\eta))$")
    plt.ylabel(r"SNR = true floor / sd(empirical floor)")
    plt.title("Approximate scaling collapse across score choices")
    plt.grid(True, alpha=0.3)
    plt.legend(handles, labels, frameon=False, fontsize=11)
    plt.tight_layout()
    plt.savefig(out_path, dpi=220)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Analyze MultiNLI conformal result zip files and plot calibration-size floor detectability.")
    parser.add_argument(
        "--zip-pattern",
        type=str,
        default="result*.zip",
    ...
    parser.add_argument(
        "--outdir",
        type=str,
        default="data_plot_multinli_output",
        help="Directory where CSVs and figures will be written for the MultiNLI run.",
    )
    parser.add_argument(
        "--alpha",
        type=float,
...
def main() -> None:
    ...
    if not zip_paths:
        raise FileNotFoundError(
            ...
            "Place this script next to the MultiNLI result_simple.zip / result_saps.zip / result_raps.zip files."
        )
    ...
    # Save lightweight dataset metadata when available.
    dataset_meta_rows = []
    for b in bundles:
        try:
            with zipfile.ZipFile(b.zip_path) as zf:
                meta_name = [n for n in zf.namelist() if n.endswith("/split_meta.json")][0]
                meta = json.loads(zf.read(meta_name).decode("utf-8"))
            dataset_meta_rows.append({
                "score": b.score_name,
                "zip_file": b.zip_path.name,
    ...
    if dataset_meta_rows:
        pd.DataFrame(dataset_meta_rows).sort_values("score").to_csv(outdir / "dataset_meta_summary.csv", index=False)
